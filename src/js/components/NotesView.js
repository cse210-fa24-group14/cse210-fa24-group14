...
export class NotesView {
  ...
  async render(note) {
    ...
    if (!note || !note.cells || note.cells.length === 0) {
      ...
      if (this.onAddCell) {
        await this.onAddCell(defaultCell.timestamp, defaultCell.content, defaultCell.cellType, null); //targetTimestamp empty for a default cell
      }
    ...
    const renderedTimestamps = Array.from(this.container.querySelectorAll('.cell-container'))
  ...
  async addCellAfterCurrent(cellContainer, cell) {
    ...
    textarea.addEventListener('input', () => {
      ...
      saveTimeout = setTimeout(() => this.onUpdateCell(cell.timestamp, textarea.value, cell.cellType), 500);
    });
  ...
  addNewCellButtons(container){
    ...
    addMarkdownButton.addEventListener('click', async () => {
      const markCell = {content: '', cellType: 'markdown', timestamp: new Date().toISOString()}
      if (this.onAddCell) {
        await this.onAddCell(markCell.timestamp, markCell.content, markCell.cellType, timestamp);
      }
    ...
    addCodeButton.addEventListener('click', async () => {
      const codeCell = {content: '', cellType: 'code', timestamp: new Date().toISOString()}
      if (this.onAddCell) {
        await this.onAddCell(codeCell.timestamp, codeCell.content, codeCell.cellType, timestamp);
      }
  ...
  async toggleCellType(event) {
    ...
    if (this.onUpdateCell) {
      await this.onUpdateCell(cell.dataset.timestamp, cellContent.value, icon.classList.contains('fa-toggle-off') ? 'markdown' : 'code');
    }
  }
  ...
  async deleteCell(event) {
    ...
    if (this.onDeleteCell) {
      try {
        ...
        const remainingCells = this.container.querySelectorAll('.cell-container');
